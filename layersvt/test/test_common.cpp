...
TEST(DispatchTableManagerTest, DispatchDownstream) {
    auto& dispatch_table_manager = DispatchTableManager::Get();
    dispatch_table_manager.Clear();
    ...
    // VkResult return type returns VK_SUCCESS fallback
    uint32_t count = 0;
    VkResult inst_result = DispatchDownstream<&VkuInstanceDispatchTable::EnumeratePhysicalDevices>(mock_instance, &count, nullptr);
    EXPECT_EQ(inst_result, VK_SUCCESS);

    VkResult dev_result = DispatchDownstream<&VkuDeviceDispatchTable::DeviceWaitIdle>(mock_device);
    EXPECT_EQ(dev_result, VK_SUCCESS);

    // void return type safely no-ops without crashing
    DispatchDownstream<&VkuInstanceDispatchTable::DestroyInstance>(mock_instance, nullptr);
    DispatchDownstream<&VkuDeviceDispatchTable::DestroyDevice>(mock_device, nullptr);
    ...
    EXPECT_EQ((DispatchDownstream<&VkuInstanceDispatchTable::EnumeratePhysicalDevices>(mock_instance, &count, nullptr)), VK_SUCCESS);
    EXPECT_EQ((DispatchDownstream<&VkuDeviceDispatchTable::DeviceWaitIdle>(mock_device)), VK_SUCCESS);
    DispatchDownstream<&VkuInstanceDispatchTable::DestroyInstance>(mock_instance, nullptr);
    DispatchDownstream<&VkuDeviceDispatchTable::DestroyDevice>(mock_device, nullptr);

    // 3. Initialized tables with valid mock function pointers (downstream forwarding)
    static bool inst_fn_called = false;
    static bool dev_fn_called = false;
    inst_fn_called = false;
    dev_fn_called = false;

    auto* inst_table = dispatch_table_manager.GetInstanceDispatchTable(mock_instance);
    ASSERT_NE(inst_table, nullptr);
    inst_table->EnumeratePhysicalDevices = [](VkInstance, uint32_t* pCount, VkPhysicalDevice*) -> VkResult {
        inst_fn_called = true;
        if (pCount) *pCount = 42;
        return VK_INCOMPLETE;
    };
    ...
    count = 0;
    EXPECT_EQ((DispatchDownstream<&VkuInstanceDispatchTable::EnumeratePhysicalDevices>(mock_instance, &count, nullptr)), VK_INCOMPLETE);
    EXPECT_TRUE(inst_fn_called);
    EXPECT_EQ(count, 42u);

    auto* dev_table = dispatch_table_manager.GetDeviceDispatchTable(mock_device);
    ASSERT_NE(dev_table, nullptr);
    dev_table->DeviceWaitIdle = [](VkDevice) -> VkResult {
        dev_fn_called = true;
        return VK_NOT_READY;
    };

    EXPECT_EQ((DispatchDownstream<&VkuDeviceDispatchTable::DeviceWaitIdle>(mock_device)), VK_NOT_READY);
    EXPECT_TRUE(dev_fn_called);
...
TEST(DispatchTableManagerTest, ConcurrentAccess) {
    ...
    std::vector<ThreadMockData> mock_data(kNumThreads);
    for (int t = 0; t < kNumThreads; ++t) {
        mock_data[t].instance_vtable = reinterpret_cast<void*>(static_cast<uintptr_t>(0x10000 + t * 0x100));
        mock_data[t].instance = reinterpret_cast<VkInstance>(&mock_data[t].instance_vtable);
        mock_data[t].device_vtable = reinterpret_cast<void*>(static_cast<uintptr_t>(0x20000 + t * 0x100));
        mock_data[t].device = reinterpret_cast<VkDevice>(&mock_data[t].device_vtable);
    }
    ...
    for (int thread_index = 0; thread_index < kNumThreads; ++thread_index) {
        threads.emplace_back([&, thread_index]() {
            ...
            for (int i = 0; i < kIterations; ++i) {
                // Initialize tables
                auto* inst_table = dispatch_table_manager.InitInstanceTable(
                    my_data.instance, [](VkInstance, const char*) -> PFN_vkVoidFunction { return nullptr; });
                EXPECT_NE(inst_table, nullptr);

                auto* dev_table = dispatch_table_manager.InitDeviceTable(
                    my_data.device, [](VkDevice, const char*) -> PFN_vkVoidFunction { return nullptr; });
                EXPECT_NE(dev_table, nullptr);
                ...
                EXPECT_EQ(dispatch_table_manager.GetInstanceDispatchTable(my_data.instance), inst_table);
                EXPECT_EQ(dispatch_table_manager.GetDeviceDispatchTable(my_data.device), dev_table);
                ...
                int neighbor_index = (thread_index + 1) % kNumThreads;
                ...
                if ((i % 10) == 0) {
                    dispatch_table_manager.DestroyDeviceTable(GetDispatchKey(my_data.device));
                    dispatch_table_manager.DestroyInstanceTable(GetDispatchKey(my_data.instance));
                }
            }
        });
    }
...
TEST(LayerManifestTest, DeviceExtensionsDownstreamIncomplete) {
    LayerManifest::Config config;
    config.layer_name = "VK_LAYER_TEST_Sample";
    // Layer defines no device extensions of its own
    LayerManifest manifest(config);

    // Mock downstream driver returning VK_INCOMPLETE
    auto mock_downstream = [](VkPhysicalDevice, const char*, uint32_t* count, VkExtensionProperties* properties) -> VkResult {
        if (!properties) {
            *count = 5;
            return VK_SUCCESS;
        }
        // Return incomplete since caller only provided space for 1
        std::strncpy(properties[0].extensionName, VK_KHR_SWAPCHAIN_EXTENSION_NAME, VK_MAX_EXTENSION_NAME_SIZE);
        properties[0].specVersion = VK_KHR_SWAPCHAIN_SPEC_VERSION;
        *count = 1;
        return VK_INCOMPLETE;
    ...
    VkResult result = manifest.EnumerateDeviceExtensionProperties(nullptr, nullptr, &count, &prop, mock_downstream);
...
TEST(LayerManifestTest, DeviceExtensionsMatchingLayerName) {
    LayerManifest::Config config;
    config.layer_name = "VK_LAYER_TEST_Sample";
    config.device_extensions = {
    ...
    uint32_t count = 0;
    VkResult result = manifest.EnumerateDeviceExtensionProperties(
    ...
    std::vector<VkExtensionProperties> extensions(count);
    result = manifest.EnumerateDeviceExtensionProperties(
...
TEST(LayerManifestTest, DeviceExtensionsForwardDifferentLayerName) {
    LayerManifest::Config config;
    config.layer_name = "VK_LAYER_TEST_Sample";
    LayerManifest manifest(config);
...
// =============================================================================
// LayerBase ProcAddr Dispatch & Template Method Tests
// =============================================================================

TEST(LayerBaseTest, ActiveLayerTracking) {
    EXPECT_EQ(LayerBase::GetActiveLayer(), nullptr);
    {
        LayerBase layer;
        EXPECT_EQ(LayerBase::GetActiveLayer(), &layer);
    }
    EXPECT_EQ(LayerBase::GetActiveLayer(), nullptr);

    LayerBase layer1;
    EXPECT_EQ(LayerBase::GetActiveLayer(), &layer1);
    LayerBase::SetActiveLayer(nullptr);
    EXPECT_EQ(LayerBase::GetActiveLayer(), nullptr);
    LayerBase::SetActiveLayer(&layer1);
    EXPECT_EQ(LayerBase::GetActiveLayer(), &layer1);
}

TEST(LayerBaseTest, GetKnownFunctionsCommonWithoutManifest) {
    LayerBase layer;
    EXPECT_NE(layer.GetKnownInstanceFunction("vkGetInstanceProcAddr"), nullptr);
    EXPECT_NE(layer.GetKnownInstanceFunction("vkCreateInstance"), nullptr);
    EXPECT_NE(layer.GetKnownInstanceFunction("vkDestroyInstance"), nullptr);
    EXPECT_NE(layer.GetKnownInstanceFunction("vkEnumeratePhysicalDevices"), nullptr);
    EXPECT_NE(layer.GetKnownInstanceFunction("vkEnumeratePhysicalDeviceGroups"), nullptr);
    EXPECT_NE(layer.GetKnownInstanceFunction("vkCreateDevice"), nullptr);

    // Without manifest, manifest-based enumeration functions return nullptr
    EXPECT_EQ(layer.GetKnownInstanceFunction("vkEnumerateInstanceExtensionProperties"), nullptr);
    EXPECT_EQ(layer.GetKnownInstanceFunction("vkEnumerateInstanceLayerProperties"), nullptr);
    EXPECT_EQ(layer.GetKnownInstanceFunction("vkEnumerateDeviceLayerProperties"), nullptr);
    EXPECT_EQ(layer.GetKnownInstanceFunction("vkEnumerateDeviceExtensionProperties"), nullptr);
    EXPECT_EQ(layer.GetKnownInstanceFunction("vkNonExistentInstanceFunction"), nullptr);
    EXPECT_EQ(layer.GetKnownInstanceFunction(nullptr), nullptr);

    EXPECT_NE(layer.GetKnownDeviceFunction("vkGetDeviceProcAddr"), nullptr);
    EXPECT_NE(layer.GetKnownDeviceFunction("vkDestroyDevice"), nullptr);
    // Instance commands must return nullptr from GetKnownDeviceFunction
    EXPECT_EQ(layer.GetKnownDeviceFunction("vkCreateDevice"), nullptr);
    EXPECT_EQ(layer.GetKnownDeviceFunction("vkEnumerateDeviceLayerProperties"), nullptr);
    EXPECT_EQ(layer.GetKnownDeviceFunction("vkEnumerateDeviceExtensionProperties"), nullptr);
    EXPECT_EQ(layer.GetKnownDeviceFunction("vkNonExistentDeviceFunction"), nullptr);
    EXPECT_EQ(layer.GetKnownDeviceFunction(nullptr), nullptr);
}

TEST(LayerBaseTest, GetKnownFunctionsCommonWithManifest) {
    LayerManifest::Config config;
    config.layer_name = "VK_LAYER_TEST_Common";
    LayerManifest manifest(config);
    LayerBase layer(&manifest);

    EXPECT_NE(layer.GetKnownInstanceFunction("vkEnumerateInstanceExtensionProperties"), nullptr);
    EXPECT_NE(layer.GetKnownInstanceFunction("vkEnumerateInstanceLayerProperties"), nullptr);
    EXPECT_NE(layer.GetKnownInstanceFunction("vkEnumerateDeviceLayerProperties"), nullptr);
    EXPECT_NE(layer.GetKnownInstanceFunction("vkEnumerateDeviceExtensionProperties"), nullptr);
    EXPECT_EQ(layer.GetKnownDeviceFunction("vkEnumerateDeviceLayerProperties"), nullptr);
    EXPECT_EQ(layer.GetKnownDeviceFunction("vkEnumerateDeviceExtensionProperties"), nullptr);
}
...
class TestDerivedLayer : public LayerBase {
   public:
    static inline auto mock_custom_inst_func = reinterpret_cast<PFN_vkVoidFunction>(0x12345678);
    static inline auto mock_custom_dev_func = reinterpret_cast<PFN_vkVoidFunction>(0x87654321);

   protected:
    PFN_vkVoidFunction GetLayerSpecificInstanceFunction(const char* name) override {
        if (std::strcmp(name, "vkCustomInstanceCmd") == 0) {
            return mock_custom_inst_func;
        }
        return nullptr;
    }

    PFN_vkVoidFunction GetLayerSpecificDeviceFunction(const char* name) override {
        if (std::strcmp(name, "vkCustomDeviceCmd") == 0) {
            return mock_custom_dev_func;
        }
        return nullptr;
    }
};
...
TEST(LayerBaseTest, LayerSpecificOverrideHooks) {
    ...
    // Custom functions handled by virtual hooks
    EXPECT_EQ(layer.GetKnownInstanceFunction("vkCustomInstanceCmd"), TestDerivedLayer::mock_custom_inst_func);
    EXPECT_EQ(layer.GetKnownDeviceFunction("vkCustomDeviceCmd"), TestDerivedLayer::mock_custom_dev_func);

    // Common functions still handled by base template method fallback
    EXPECT_NE(layer.GetKnownInstanceFunction("vkCreateInstance"), nullptr);
    EXPECT_NE(layer.GetKnownInstanceFunction("vkCreateDevice"), nullptr);
    EXPECT_NE(layer.GetKnownDeviceFunction("vkDestroyDevice"), nullptr);
    EXPECT_EQ(layer.GetKnownDeviceFunction("vkCreateDevice"), nullptr);

    // Unhandled functions return nullptr
    EXPECT_EQ(layer.GetKnownInstanceFunction("vkUnknownCmd"), nullptr);
    EXPECT_EQ(layer.GetKnownDeviceFunction("vkUnknownCmd"), nullptr);
}
...
TEST(LayerBaseTest, ProcAddrDispatchChain) {
    ...
    EXPECT_NE(layer.GetInstanceProcAddr(VK_NULL_HANDLE, "vkGetInstanceProcAddr"), nullptr);
    EXPECT_NE(layer.GetInstanceProcAddr(VK_NULL_HANDLE, "vkCreateInstance"), nullptr);

    // Non-global commands must return nullptr when instance is VK_NULL_HANDLE
    EXPECT_EQ(layer.GetInstanceProcAddr(VK_NULL_HANDLE, "vkDestroyInstance"), nullptr);
    EXPECT_EQ(layer.GetInstanceProcAddr(VK_NULL_HANDLE, "vkCreateDevice"), nullptr);
    EXPECT_EQ(layer.GetInstanceProcAddr(VK_NULL_HANDLE, "vkCustomInstanceCmd"), nullptr);
    EXPECT_EQ(layer.GetInstanceProcAddr(VK_NULL_HANDLE, "vkCustomDeviceCmd"), nullptr);
    EXPECT_EQ(layer.GetInstanceProcAddr(VK_NULL_HANDLE, "vkNextLayerCmd"), nullptr);

    // GetDeviceProcAddr with VK_NULL_HANDLE must always return nullptr
    EXPECT_EQ(layer.GetDeviceProcAddr(VK_NULL_HANDLE, "vkGetDeviceProcAddr"), nullptr);
    EXPECT_EQ(layer.GetDeviceProcAddr(VK_NULL_HANDLE, "vkDestroyDevice"), nullptr);
    EXPECT_EQ(layer.GetDeviceProcAddr(VK_NULL_HANDLE, "vkCreateDevice"), nullptr);
    EXPECT_EQ(layer.GetDeviceProcAddr(VK_NULL_HANDLE, "vkCustomDeviceCmd"), nullptr);
    EXPECT_EQ(layer.GetDeviceProcAddr(VK_NULL_HANDLE, "vkNextLayerCmd"), nullptr);

    // 2. Querying with valid instance handle
    void* mock_instance_vtable = reinterpret_cast<void*>(0x11223344);
    auto mock_instance = reinterpret_cast<VkInstance>(&mock_instance_vtable);
    static auto mock_next_inst_cmd = reinterpret_cast<PFN_vkVoidFunction>(0xABCDEF01);

    dispatch_table_manager.InitInstanceTable(mock_instance, [](VkInstance, const char* function_name) -> PFN_vkVoidFunction {
        if (std::strcmp(function_name, "vkNextLayerInstCmd") == 0) {
            return mock_next_inst_cmd;
        }
        return nullptr;
    });

    EXPECT_EQ(layer.GetInstanceProcAddr(mock_instance, "vkCustomInstanceCmd"), TestDerivedLayer::mock_custom_inst_func);
    EXPECT_EQ(layer.GetInstanceProcAddr(mock_instance, "vkCustomDeviceCmd"), TestDerivedLayer::mock_custom_dev_func);
    EXPECT_NE(layer.GetInstanceProcAddr(mock_instance, "vkCreateDevice"), nullptr);
    EXPECT_NE(layer.GetInstanceProcAddr(mock_instance, "vkDestroyInstance"), nullptr);
    EXPECT_EQ(layer.GetInstanceProcAddr(mock_instance, "vkNextLayerInstCmd"), mock_next_inst_cmd);
    EXPECT_EQ(layer.GetInstanceProcAddr(mock_instance, "vkUnimplementedCmd"), nullptr);

    // 3. Querying with valid device handle
    void* mock_device_vtable = reinterpret_cast<void*>(0x55667788);
    auto mock_device = reinterpret_cast<VkDevice>(&mock_device_vtable);
    static auto mock_next_dev_cmd = reinterpret_cast<PFN_vkVoidFunction>(0xABCDEF02);
    ...
    dispatch_table_manager.InitDeviceTable(mock_device, [](VkDevice, const char* function_name) -> PFN_vkVoidFunction {
        if (std::strcmp(function_name, "vkNextLayerDevCmd") == 0) {
            return mock_next_dev_cmd;
        }
        return nullptr;
    });

    EXPECT_EQ(layer.GetDeviceProcAddr(mock_device, "vkCustomDeviceCmd"), TestDerivedLayer::mock_custom_dev_func);
    EXPECT_NE(layer.GetDeviceProcAddr(mock_device, "vkGetDeviceProcAddr"), nullptr);
    EXPECT_NE(layer.GetDeviceProcAddr(mock_device, "vkDestroyDevice"), nullptr);
    // Instance commands must return nullptr via GetDeviceProcAddr even with valid device
    EXPECT_EQ(layer.GetDeviceProcAddr(mock_device, "vkCreateDevice"), nullptr);
    EXPECT_EQ(layer.GetDeviceProcAddr(mock_device, "vkDestroyInstance"), nullptr);
    EXPECT_EQ(layer.GetDeviceProcAddr(mock_device, "vkNextLayerDevCmd"), mock_next_dev_cmd);
    EXPECT_EQ(layer.GetDeviceProcAddr(mock_device, "vkUnimplementedCmd"), nullptr);
}
